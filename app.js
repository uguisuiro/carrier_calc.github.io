...
function drawChart(planNetPrice, optionNetPrice, initialCost, device, applyBenefit, isUpfront) {
    ...
    const maxMonths = (paymentType === "program") ? currentTotalCount : customCount;
    const graphMonths = Math.max(24, maxMonths);
    ...
    for (let m = 1; m <= graphMonths; m++) {
        labels.push(m + "か月目");
        baseCosts.push(planNetPrice + optionNetPrice);
        if (m == 1) {
            initialCosts.push(initialCost);
        } else {
            initialCosts.push(0);
        }
        let monthlyDevice = 0;
        if (paymentType === "program") {
            monthlyDevice = getMonthlyDevicePrice(m, device, applyBenefit, isUpfront);
        } else if (paymentType === "normal") {
            monthlyDevice = (m <= customCount) ? normalMonthly : 0;
        } else {
            monthlyDevice = 0;
        }
        if (m <= selectedReturnMonth) {
            deviceCosts.push(monthlyDevice);
            overtimeCosts.push(0);
        } else {
            deviceCosts.push(0);
            overtimeCosts.push(monthlyDevice);
        }
    }
...
(分割手数料 ${fmt(currentManualFee)} 円含む)
    ...
    drawChart(planNetPrice, optionNetPrice, (finalDown + 3850 + pgFee), device, applyBenefit, isUpfront);
}
